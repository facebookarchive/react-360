...
const remoteBranch = `https://${GIT_USER}@github.com/facebook/react-vr.git`;
...
if (!CI_PULL_REQUEST && CIRCLE_PROJECT_USERNAME === `facebook`) {
  ...
  cd(`build`);
  rm(`-rf`, `react-vr-gh-pages`);

  if (exec(`git clone ${remoteBranch} react-vr-gh-pages`).code !== 0) {
    echo(`Error: Git clone failed`);
    exit(1);
  }

  cd(`react-vr-gh-pages`);
  ...
  if (areVersionlessSectionsToBeDeployed) {
    ...
    cp(`../react-vr/showcase.html`, `.`);
    echo(`------------ COPYING support`);
    cp(`../react-vr/support.html`, `.`);
  }
