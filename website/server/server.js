...
var app = connect()
  .use(function(req, res, next) {
    ...
    if (!server.noconvert && req.url.match(/\.html|\/$/)) {
      var extractDocs = req.url.match(/\/docs/);
      convert({extractDocs});
    }
    next();
  })
  ...
  .use(connect.favicon(path.join(FILE_SERVE_ROOT, 'react-vr', 'img', 'favicon.png')))
  .use(connect.logger())
...
server.listen(portToUse, function(){
  console.log('Open http://localhost:' + portToUse + '/react-vr/index.html');
});
