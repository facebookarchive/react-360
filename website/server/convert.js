...
var siteSidebar = require('./siteSidebar');
...
function execute(options) {
  ...
  glob.sync('src/react-vr/docs/*.*').forEach(rmFile);
  glob.sync('src/react-vr/blog/*.*').forEach(rmFile);
  glob.sync('../blog/img/*.*').forEach(file => {
    writeFileAndCreateFolder(
      'src/react-vr/blog/img/' + path.basename(file),
  ...
  function findMetadata(file) {
    for (var i=0; i<metadatas.files.length; i++) {
      if (metadatas.files[i].filename === file) {
        return metadatas.files[i];
      }
    }
  }

  function relink() {
    for (var i=0; i<siteSidebar.markdownOrder.length; i++) {
      var md = findMetadata(siteSidebar.markdownOrder[i]);
      if (md) {
        md.category = siteSidebar.findCategory(siteSidebar.markdownOrder[i]);
        if (i>0) {
          var prev = findMetadata(siteSidebar.markdownOrder[i-1]);
          md.previous = prev ? prev.id : null;
        }
        if (i<siteSidebar.markdownOrder.length-1) {
          var next = findMetadata(siteSidebar.markdownOrder[i+1]);
          console.log('replace next ' + md.next + ' ' + next.id )
          md.next = next ? next.id : null;
        } else {
          md.next = 'view';
        }
      }
  ...
  function handleMarkdown(content, filename) {
    ...
    if (metadata && filename) {
      ...
      let i = siteSidebar.findIndex(filename);
      if (i !== -1) {
        // getting started is an anchor, don't amend the file
        if (filename !== 'GettingStarted.md') {
          metadata.id = makeid(filename);
          metadata.permalink = 'docs/' + metadata.id + '.html';
        }
        metadata.category = siteSidebar.findCategory(filename);
        if (i>0) {
          var prev = makeid(siteSidebar.markdownOrder[i-1]);
          metadata.previous = prev ? prev : null;
        }
        if (i<siteSidebar.markdownOrder.length-1) {
          var next = makeid(siteSidebar.markdownOrder[i+1]);
          metadata.next = next ? next : null;
        } else {
          metadata.next = 'view';
        }
      }
    ...
    writeFileAndCreateFolder(
      'src/react-vr/' + metadata.permalink.replace(/\.html$/, '.js'),
  ...
  files.sort().reverse().forEach(file => {
    ...
    writeFileAndCreateFolder(
      'src/react-vr/blog/' + filePath.replace(/\.html$/, '.js'),
  ...
  for (var page = 0; page < Math.ceil(metadatasBlog.files.length / perPage); ++page) {
    writeFileAndCreateFolder(
      'src/react-vr/blog' + (page > 0 ? '/page' + (page + 1) : '') + '/index.js',
