...
export default class ThreeDOFRayCaster extends RayCaster {
  ...
  frame() {
    ...
    if (gamepad && gamepad.pose) {
      if (gamepad.pose.hasOrientation) {
  ...
  getRayOrigin(camera) {
    ...
    const controllerPosition = this._mesh.position
    return [
      controllerPosition.x - camera.position.x,
      controllerPosition.y - camera.position.y,
      controllerPosition.z - camera.position.z,
    ]
  }
