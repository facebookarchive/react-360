    ...
    switch (key) {
      ...
      case 'scale':
        if (typeof value === 'number') {
          ...
          _multiplyTransform(result, MatrixMath.reuseScale3dCommand, [
            value[0],
            value[1],
            value[2],
          ]);
...
function _convertToRadians(value: string): number {
  const isRad = typeof value === 'string' ? value.indexOf('rad') > -1 : false;
  const floatValue = typeof value === 'string' ? parseFloat(value, 10) : value;
