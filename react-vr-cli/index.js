...
getLatestVersion().then((version) => {
  if (version && version !== CURRENT_VERSION) {
    ...
    console.log(chalk.red(`Current CLI version is ${CURRENT_VERSION}, but the latest available version is ${version}`));
  ...
  if (args[0] === 'init') {
    ...
    try {
      ...
      if (stat) {
        console.log(
          'A ' + (stat.isDirectory() ? 'directory' : 'file') + ' named ' + rawName +
    ...
    generateFiles(source, dest, {name}).then(() => {
        ...
        cmd.on('close', (code) => {
          if (code !== 0) {
            console.log(
              ...
              '. Try fixing the errors above and run `' + cmdName + '` from within ' +
              name + ' again.'
            );
            process.exit(1);
          }
      ...
      console.log('Done!');
      console.log('  Now enter the new project directory by running ' + chalk.green('`cd ' + rawName + '`'));
      console.log('  Run ' + chalk.green('`npm start`') + ' to initialize the development server');
