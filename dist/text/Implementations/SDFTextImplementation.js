...
export default class SDFTextImplementation {
  ...
  extractGlyphs(fontName, size, text, color = 0xff000000) {
    const run = {
      glyphs: [],
      maxAscend: 0,
      maxDescend: 0,
    ...
    for (let i = 0, length = text.length; i < length; i++) {
      ...
      if (charCode === COLOR_MARKER) {
        currentColor = 0xff000000 | (text.charCodeAt(i + 1) & 0xff) << 16 | (text.charCodeAt(i + 2) & 0xff) << 8 | text.charCodeAt(i + 3) & 0xff;
        i += 4;
        continue;
      }
  ...
  updateGeometryAndMaterial(geometry, material, info, params) {
    const {
      align
    } = params;
    ...
    for (let i = 0; i < info.lines.length; i++) {
      ...
      for (let j = 0; j < glyphs.length; j++) {
        ...
        const floatStart = vertIndex * 6; // top left
        ...
        uintBuffer[uintStart + 95] = alpha; // prettier-ignore
    ...
    if ('a_position' in attributes) {
      ...
      geometry.addAttribute('a_position', new THREE.InterleavedBufferAttribute(attrBuffer32, 2, 0, false));
    }
    ...
    if ('a_uv' in attributes) {
      ...
      geometry.addAttribute('a_uv', new THREE.InterleavedBufferAttribute(attrBuffer32, 2, 2, false));
    }
    ...
    if ('a_center' in attributes) {
      ...
      geometry.addAttribute('a_center', new THREE.InterleavedBufferAttribute(attrBuffer32, 1, 4, false));
    }
    ...
    if ('a_color' in attributes) {
      ...
      geometry.addAttribute('a_color', new THREE.InterleavedBufferAttribute(attrBuffer8, 4, 20, true));
    }
